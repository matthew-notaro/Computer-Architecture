...
void prefetch(unsigned int);    // int: address
...
int main(int argc, char** argv){
    ...
    numBlockBits = log2(blockSize), numSetBits = log2(setSize);
...
void prefetch(unsigned int prefetchAddress){
    int i;
    unsigned int index;
    unsigned long tag;
    for(i = 0; i < prefetchSize; i++){
        prefetchAddress += blockSize;
	index = (prefetchAddress >> numBlockBits) & ((1 << numSetBits) - 1);
	tag = prefetchAddress >> (numBlockBits + numSetBits);
